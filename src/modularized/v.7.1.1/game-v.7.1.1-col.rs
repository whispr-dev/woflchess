...
impl fmt::Display for GameState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Use format strings with "{}", then the colored text.
        writeln!(f, "{}", "\n  a b c d e f g h".cyan())?;
        writeln!(f, "{}", "  ---------------".magenta())?;

        for rank in (0..8).rev() {
            // Print rank number at start of the line
            write!(f, "{} ", rank + 1)?;
            ...
            // Print rank number again at end of line
            writeln!(f, "{}", rank + 1)?;
        }

        writeln!(f, "{}", "  ---------------".yellow())?;
        writeln!(f, "{}", "  a b c d e f g h".blue())?;

        // Show which color is to move

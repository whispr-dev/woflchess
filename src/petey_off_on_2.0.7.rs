...
struct CAInterface {
    ...
    height: usize,
    cells: Vec<Vec<f32>>,
    // Add `Send + Sync + 'static`:
    update_rules: Vec<Box<dyn Fn(&[f32]) -> f32 + Send + Sync + 'static>>,
}
...
////////////////////////////////////////////////////////////
fn main() {
    // Make a shared game state
    let shared_gs = Arc::new(Mutex::new(GameState::new()));
    // Extract the neural engine, etc.
    let engine_arc = {
    ...
    let shared_gs_for_console = Arc::clone(&shared_gs);
    std::thread::spawn(move || {
        console_loop(shared_gs_for_console, engine_arc);
    });

    // Now run the GUI in the main thread
    let app = ChessApp {
        selected_square: None,
        shared_gs: Arc::clone(&shared_gs),
    };
    let options = eframe::NativeOptions::default();
    eframe::run_native("Chess GUI", options, Box::new(|_cc| Box::new(app)));

    // Once the GUI closes, main thread ends => the program ends
    // or if you want to wait for console thread, store the handle & join
}

fn console_loop(shared_gs: Arc<Mutex<GameState>>, engine_arc: Arc<Mutex<ChessNeuralEngine>>) {
    loop {
        println!("Enter command (train time X, train games X, etc)...");
...
fn play_game(shared_gs: Arc<Mutex<GameState>>) {
    ...
    loop {
      println!("{}", game_state);
      let status = game_state.get_game_status();
      println!("{}", status);
  
      // If status says "Checkmate" or "Stalemate", break
